...
public class Exam1227_2 {
	...
	public int[] solution(int[] array, int[][] commands) {
        ...
        for(int i=0;i<commands.length;i++) { // 반복
        	int[] ar = new int[commands[i][1]-commands[i][0]+1]; // 잘라낼 개수 = (두번째수 - 첫번째수 + 1)
        	//               원본,  시작위치ㅣ, 사본, 복사위치, 개수
        	System.arraycopy(array, commands[i][0]-1, ar, 0, commands[i][1]-commands[i][0]+1);
        	Arrays.sort(ar); // 정렬
        	answer[i] = ar[commands[i][2]-1]; // 세번째수가 가져올 위치 : 배열이므로 -1해야 한다.
        }
        return answer;
    }
	...
	@Test
	public void test() {
		assertArrayEquals(new Exam1227_2().solution(new int[] {1, 5, 2, 6, 3, 7, 4}, 
